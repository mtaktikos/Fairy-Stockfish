...
namespace {
  ...
  ExtMove* generate_pawn_moves(const Position& pos, ExtMove* moveList, Bitboard target) {
    ...
    if (Type != CAPTURES)
    {
        Bitboard b1 = shift<UpRight>(pawnsNotOn7) & emptySquares;
		Bitboard b2 = shift<UpLeft>(pawnsNotOn7) & emptySquares;
		Bitboard b3 = pos.double_step_enabled() ? shift<UpRight>(b1 & TRank3BB) & emptySquares : Bitboard(0);
		Bitboard b4 = pos.double_step_enabled() ? shift<UpLeft>(b2 & TRank3BB) & emptySquares : Bitboard(0);
        ...
        if (Type == QUIET_CHECKS && pos.count<KING>(Them))
        {
            ...
            b1 &= pawn_attacks_bb(Them, ksq) | shift<   UpRight>(dcCandidatePawns);
			b2 &= pawn_attacks_bb(Them, ksq) | shift<   UpLeft>(dcCandidatePawns);
			b3 &= pawn_attacks_bb(Them, ksq) | shift<UpRight+UpRight>(dcCandidatePawns);
			b4 &= pawn_attacks_bb(Them, ksq) | shift<UpLeft+UpLeft>(dcCandidatePawns);
        }
		...
		while (b3)
        {
            ...
            *moveList++ = make_move(to - UpRight -UpRight, to);
        }
        while (b4)
        {
            Square to = pop_lsb(b4);
            *moveList++ = make_move(to - UpLeft -UpLeft, to);
        }
    ...
    if (pawnsOn7)
    {
        Bitboard b1 = shift<Up>(pawnsOn7) & enemies;
        Bitboard b2 = shift<UpRight >(pawnsOn7) & emptySquares;
        Bitboard b3 = shift<UpLeft  >(pawnsOn7) & emptySquares;
        ...
        while (b1)
            moveList = make_promotions<Us, Type, Up>(pos, moveList, pop_lsb(b1));

        while (b2)
            moveList = make_promotions<Us, Type, UpRight >(pos, moveList, pop_lsb(b2));

        while (b3)
            moveList = make_promotions<Us, Type, UpLeft   >(pos, moveList, pop_lsb(b3));
    }
    ...
    if (Type == CAPTURES || Type == EVASIONS || Type == NON_EVASIONS)
    {
        Bitboard b1 = shift<Up>(pawnsNotOn7) & enemies;
        ...
        if (pos.ep_square() != SQ_NONE)
        {
            ...
            //if (Type == EVASIONS && 
			//((target & (pos.ep_square() + UpRight))||(target & (pos.ep_square() + UpLeft)))
			//)
			
			if (Type == EVASIONS && 
			(target & (pos.ep_square() + Up))
			)
                return moveList;
